            ...
            <div>
                ...
                <svg class="icon icon-xs" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
                        ...
                        <a href="login-example" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
                ...
                <div class="row">
                    <div class="col-12 col-xl-8">
                        ...
                        <div class="card card-body border-0 shadow mb-4">
                            ...
                            <form action="{{ route('neighbour.store') }}" id="myform" method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="tid" value="{{ $editId }}">
                                <!--  -->
                                <div class="row">
                                    <div class="col-md-6">
                                        <h2 class="h5 mb-4">My Neighbourhood</h2>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    <div class="my-3">
                                        ...
                                        <select id="source" name="source" class="form-select" required>
                                            ...
                                            <option value="{{ $domain->id }}" {{ $source == $domain->id ? 'selected' : '' }}>
                                                {{ $domain->domain_name }}
                                            </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="my-3">
                                        ...
                                        <select id="target" name="target" class="form-select" required>
                                            <option value="Select" disabled selected>Select</option>
                                            @foreach(App\Models\Domain::all() as $domain)
                                            <option value="{{ $domain->id }}" {{ $target == $domain->id ? 'selected' : '' }}>
                                                {{ $domain->domain_name }}
                                            </option>
                                            @endforeach
                                        </select>
